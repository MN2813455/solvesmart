...
const STORAGE_KEY = 'rationalist_current_session';
...
function App() {
  ...
  // Persistence: Save to storage when data changes
  useEffect(() => {
    if (reportData) {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] flex flex-col font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
      {/* Modern Header */}
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-100 h-20 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-50">
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setView('chat')}>
          ...
          <span className="font-black text-xl sm:text-2xl text-slate-900 tracking-tighter">Rationalist</span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {reportData && view !== 'settings' && (
            <button 
              ...
              className="text-[10px] sm:text-sm font-bold text-indigo-600 hover:text-indigo-700 px-3 sm:px-4 py-2 rounded-2xl bg-indigo-50 hover:bg-indigo-100/50 transition-all flex items-center gap-2 border border-indigo-100/50"
            >
              <Icon name={view === 'chat' ? 'FileText' : 'MessageSquare'} size={16} />
              <span className="hidden xs:inline">{view === 'chat' ? 'Strategy' : 'Discussion'}</span>
            </button>
          )}
          
          <div className="h-8 w-px bg-slate-100 mx-1 sm:mx-2 hidden xs:block"></div>
      ...
      <main className="flex-grow flex flex-col overflow-y-auto">
        {view === 'settings' ? (
          ...
          <div className="animate-fade-in flex flex-col pb-20">
            ...
            <div className="max-w-5xl mx-auto w-full px-4 sm:px-6 mt-12 mb-32">
               <div className="bg-white rounded-[32px] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                  <div className="px-6 sm:px-8 py-4 sm:py-6 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                     <div className="flex items-center gap-4">
                        <div className="p-3 bg-indigo-600 rounded-2xl text-white shadow-md">
                           <Icon name="MessageSquare" size={20} />
                        </div>
                        <div>
                           <h3 className="font-extrabold text-slate-900 text-base sm:text-lg">Refinement Loop</h3>
                           <p className="text-[10px] sm:text-sm text-slate-500 font-medium">Add constraints or new data to evolve the plan.</p>
                        </div>
                     </div>
                  </div>
                  <div className="h-[600px] sm:h-[750px] max-h-[80vh]">
          ...
          <div className="flex-grow flex flex-col items-center justify-center p-0">
             <ChatSession onComplete={handleAnalysisComplete} initialData={reportData} />
          </div>
        )}
      </main>
